<template>
  <div class="s-cascader">
    <div class="selected" @click="showOptions" :class="{active:optionsVisible}">
      ...
      <s-icon name="close" v-if="clearable" class="x-icon close" v-show="selected.length" @click.stop="clearSelected"></s-icon>
      <span class="x-icon" v-else :class="{active:optionsVisible}">></span>
      <!-- <s-icon name="user" class="x-icon" :class="{active:optionsVisible}"></s-icon> -->
    </div>
